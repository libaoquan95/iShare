...
<head>
	...
	<title>搜索/高级搜索 - iShare 网址压缩分享平台</title>
...
<body>
	...
	<div id='main_bk'>
			...
			<div id='in_url_list'>
				...
				<div class='url_desc_block'>
					<p class='url_user'>
						...
						<a href='/user/{{ $user->name }}'><img src='{{ $user->user_img }}'	title='用户：{{ $user->name }}' alt='{{ $user->name }}' class='users_img' align="middle"/>
						<span>{{ $user->name }}的分享</span></a>
						@endforeach
		...
		<div id='more_search'>
			<p>当前搜索</p>
			<p>网址描述信息：<span id="search_desc">{{ $info['search_info']['search_desc'] }}</span></p>
			<p>原网址：<span id="search_lurl">{{ $info['search_info']['search_lurl'] }}</span></p>
			<p>短网址：<span id="search_surl">{{ $info['search_info']['search_surl'] }}</span></p>
	...
	<script type="text/javascript" src="/js/load_next_more_search_page.js"></script>
	<script>
		// 遍历
