...
<head>
	...
	<title>话题广场/{{$info['topic_name']}} iShare  - iShare 网址压缩分享平台</title>
...
<body>
	...
	<div id='header'>
		...
		<ul class='order_list'>
			<a href='/topics/{{ $info['topic_name'] }}'><li id="new_list">最新</li> <a/>
			<a href='/topics/{{ $info['topic_name'] }}/hot'><li id="hot_list" class="local_order">最热</li> <a/>
		</ul>
		...
		<ul class='menu_list'>
			...
			<a href='/topics/{{ $info['topic_name'] }}' id='local_page'>
				<li>
				<img src='/css/img/head_list_topic.png' alt='' class='head_list_img'/>话题广场：{{ $info['topic_name'] }}</li>
			</a>
	...
	<div id='main_bk'>
		...
		<div id='topic_ground'>
			<div class='topic_ground_bar'>
				<p class='topic_title'>话题：{{$info['topic_name']}} 
					<span> (
						@if ( $info['topic_urlCount'] > 10000)
							{{ $info['topic_urlCount']/10000 }}万
						@else
							{{ $info['topic_urlCount'] }}
						@endif
						个信息)
					</span>
					<img src="{{ $info['topic_img'] }}" title="话题：{{$info['topic_name']}}" alt="{{$info['topic_name']}} " class='topics_img_in_topicPage'/>
				</p>
			</div>
			...
			<input type="hidden" id="topic_name" name="topic_name" value='{{ $info['topic_name'] }}' />
		</div>
	</div>
	<div id='footer'>
		<ul>
	...
	<script type="text/javascript" src="/js/load_topic_next_hot_page.js"></script>
	<script type="text/javascript">
